    ...
    <div class="paper-create">
      ...
      <div class="two-columns">
        ...
        <el-card class="right-panel" shadow="never">
          ...
          <div v-for="(q, index) in selectedQuestions" :key="q.id" class="paper-question">
            <div class="q-header">
...
function addQuestion(q) {
  ...
  selectedQuestions.value.push({ ...q, score: 5 })
  calcTotal()
}
...
function saveDraft() {
  ...
  saving.value = true
  createPaper(buildPayload()).then(res => {
...
function validateAndSubmit() {
  ...
  createPaper(buildPayload()).then(res => {
    ...
    // 调用校验接口
    const { validatePaper } = require('../api/paper')
    validatePaper(paperId).then(vres => {
      validationResult.value = vres.data
      showValidation.value = true
      // 保存当前试卷id供提交使用
      validationResult.value._paperId = paperId
    })
  }).finally(() => { saving.value = false })
}
...
function confirmSubmit() {
  ...
  if (!paperId) return
  saving.value = true
  const { submitPaper } = require('../api/paper')
...
<style scoped>
.paper-create { max-width: 1600px; margin: 0 auto; }
